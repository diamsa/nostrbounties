...
function CreateBounty() {
  ...
  let [title, setTitle] = useState<string>("Implement a widget like the remoteStorage widget for NIP-78");
  let [content, setContent] = useState<string>("https://remotestorage.io/ provides a nice widget that apps can embed. It floats on the side of the screen and users can click there to login to their selected remoteStorage provider. We could have a similar flow for users to pick their Nostr relays. Then the widget would offer a nice JavaScript API for the app developers to call, abstracting the Nostr stuff and just offering .get() and .set(), I don't know. And also encrypting the content in between automatically using some reasonable and simple encryption algorithm.\n\nIt should rely on NIP-07 or on users pasting their private keys directly.");
  let [reward, setReward] = useState<string>("150000");
