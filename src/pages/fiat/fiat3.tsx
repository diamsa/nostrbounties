...
function CreateBounty() {
  ...
  let [title, setTitle] = useState<string>("Write a command line tool for sending git patches over Nostr");
  let [content, setContent] = useState<string>("Essentially http://git.jb55.com/git-nostr-tools/file/README.txt.html, but in a self-contained binary that is easy to install (using Go or Rust -- which are the only languages I know that can give you this) with no external dependencies. It should display and handle nevent and nprofile with autoconfiguring relays for each (the idea is that a person will be able to send a patch to another, pick the relays they want to use at publish time or from a git config, and message the patch receiver with the nevent code for the patch and they will be able to fetch that from that specific relay and apply). It should be a git subcommand.");
  let [reward, setReward] = useState<string>("200000");
