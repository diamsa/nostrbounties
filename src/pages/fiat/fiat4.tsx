...
function CreateBounty() {
  ...
  let [title, setTitle] = useState<string>("NIP dependency tree");
  let [content, setContent] = useState<string>("Go through all NIPs and add depends:* tags (like NIP-51 has one) to them referencing all the NIPs they depend on (multiple tags, not a single tag with multiple dependencies), then write a GitHub Actions script that will parse all the NIPs and produce a .dot file and render a dependency tree with GraphViz, then publish it to GitHub and display it on the repository README.");
  let [reward, setReward] = useState<string>("90000");
