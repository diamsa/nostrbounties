...
function CreateBounty() {
  ...
  let [title, setTitle] = useState<string>("An web app for making lists");
  let [content, setContent] = useState<string>("A very simple and functional web Nostr client for making lists, for example: * lists of best websites of a certain topic * lists of the best Nostr notes (kind 1) * lists of the best Nostr articles (kind 30023) * lists of the best Nostr people of a certain topic * lists of the best relays for each topic It should probably look like https://pinboard.in/ and use NIP-51 for the list formats. Adding tags to the lists but also listing all the things as tags, with relay hints and comments for each item. The idea is that people will make these lists and others will be able to browse their lists and visit their bookmarked content using nprofile codes to find these people, or using naddr codes to share and browse specific lists.");
  let [reward, setReward] = useState<string>(" 1000000");
